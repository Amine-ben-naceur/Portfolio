...
import ProjectManifestation from "./ProjectManifestation";
...
function ProjectCard({ project, index, step, featuredLabel, cardGeo, glowGeo }: CardProps) {
...
export default function ProjectOrbit() {
  ...
  // Counter-rotate the ring tilt so manifestations (dolphin/elephant/lotus) stand upright
  const INV_RING_TILT_Q = useMemo(() => {
    const q = new THREE.Quaternion();
    q.setFromEuler(PROJECTS_RING_TILT);
    q.invert();
    return q;
  }, []);
  ...
  return (
    <group
      ...
      visible={false}
    >
      {/* Giant project manifestations (dolphin / elephant / lotus) */}
      <group quaternion={INV_RING_TILT_Q}>
      ...
      <group ref={carouselRef}>
        {projects.map((project, i) => (
          <ProjectCard
            ...
            index={i}
            step={step}
            featuredLabel={ui.projects.featuredBadge}
            cardGeo={cardGeo}
            glowGeo={glowGeo}
          />
        ))}
