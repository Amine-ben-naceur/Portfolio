...
export default function HUDRail() {
  ...
  // useScrollRaf subscribes once on mount (see scroll.ts) — the callback's
  // closure would otherwise go stale after a language toggle, so the
  // per-frame readout reads through a ref that's kept in sync instead.
  ...
  useScrollRaf((p, v) => {
    ...
    const sec = `SEC // ${labelsRef.current[sectionAt(p)].toUpperCase()}`;
        ...
        {SECTIONS.map((s) => {
          ...
          return (
            <button
              ...
              type="button"
              data-cursor="hover"
              aria-label={ui.hudRail.goTo(labels[s.id])}
